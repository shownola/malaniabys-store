...
PRODUCTS_COUNT.times do
  ...
  product = Product.new(title: title, price: Faker::Commerce.price)
  ...
  if rand > VARIANTS_WEIGHT
    ...
    num_variants.times do
      product.variants.build(title: rand > VARIANTS_WEIGHT ? Faker::Commerce.color : Faker::Commerce.material, price: Faker::Commerce.price)
    end
  end
